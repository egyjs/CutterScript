            ...
            if(empty($urlInput)){
                $this->msg = '<span><div class="alert alert-warning" role="alert">INSERT A URl TO SHORT IT</div></span>';
                    ...
                    $this->url->shortURL($urlInput,$url_code);
                        ...
                        if($this->url->shortforUser($urlInput,$url_code,$_SESSION['userSession'])){
                            $id_action = $this->url->lastID;
                            $this->user_login->addToTimeline("add","links",$_SESSION['userSession'],$id_action);
                        }
                    }
