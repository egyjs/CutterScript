     ...
     public function login($uemail,$upass)
     {
      try{
       ...
       echo $ex->getMessage();
          ...
          public function addToTimeline($action,$tblname,$uid,$id_action) {
        try {
            
            $time = date("Y-m-d h:i:s");
            $stmt = $this->conn->prepare("INSERT INTO timeline(t_action,t_tblname,u_id,id_action,t_time) 
                                                         VALUES(:action,:tblname,:UsrID,:idaction,:time)");
            $stmt->bindparam(":action",$action);
            $stmt->bindparam(":tblname",$tblname);
            $stmt->bindparam(":UsrID",$uid);
            $stmt->bindparam(":idaction",$id_action);
